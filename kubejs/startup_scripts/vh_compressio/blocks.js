...
onEvent('block.registry', event => {
    event.create('the_vault:vault_plating_block').displayName('Блок покрытия свода').tagBlock('minecraft:mineable/pickaxe')
    event.create('the_vault:carbon_block').displayName('Блок углерода').tagBlock('minecraft:mineable/pickaxe')
    vanillaStandardCompressed.forEach(block => {
        ...
        let finalizedDisplayName = `${integerOfCompression}x Сжатый ${splitId.join(' ').slice(0, -2)}`;
